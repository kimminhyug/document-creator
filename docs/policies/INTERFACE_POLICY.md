...
PDF 테마의 `layout.keep_title_words`는 표지 제목의 단어 단위 줄바꿈, `table.column_width_mode`는 `equal`(동일 폭) 또는 `content`(내용 길이 기반 열 너비)를 정한다. 긴 표는 페이지를 나눠 반복 헤더를 붙이며 짧은 절의 묶음 높이는 `layout.keep_short_sections_max_height_mm`로 정한다. 이 설정도 모든 실제 출력 페이지 검수를 대신하지 않는다.
